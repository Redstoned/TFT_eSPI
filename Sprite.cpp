...
/***************************************************************************************
** Function name:           scrollWrapX
** Description:             Scroll dx, positive right, negative left, with wrapping
*************************************************************************************x*/
void TFT_eSprite::scrollWrapX(int16_t dx)
{
  ...
  // Fetch the scroll area width and height set by setScrollRect()
  uint32_t w  = _sw - absdx; // line width to copy
  uint32_t h  = _sh; // lines to copy
  ...
  {		
	...
	if(dx <= 0)
	{
		...
		{
			for(uint16_t ix = 0; ix < absdx; ix ++)
			{
				wrap[ix][iy] = readPixel(ix, iy);
			}
		}	
		//  scroll
		while(h--)
		{
			for(uint16_t xp = 0; xp < w - absdx; xp ++)
			{
		...
		for(uint16_t iy = 0; iy < hh; iy ++)
		{
			for(uint16_t ix = 0; ix < absdx; ix ++)
			{
				drawPixel(w - absdx + ix, iy, wrap[ix][iy]);
			}
		}		
	}
	else
	{
		...
		for(uint16_t iy = 0; iy < h; iy ++)
		{
			for(uint16_t ix = 0; ix < absdx; ix ++)
			{
				wrap[ix][iy] = readPixel(w - dx + ix, iy);
			}
		}
		//  scroll
		while(h--)
		{
			for(uint16_t xp = absdx + 1; xp < w + 1; xp ++)
			{
				drawPixel(tx - xp, ty, readPixel(fx - xp, fy));
			}			
			ty++;
...
/***************************************************************************************
** Function name:           scrollWrapY
** Description:             Scroll dy, positive down, negative up, with wrapping
*************************************************************************************x*/
void TFT_eSprite::scrollWrapY(int16_t dy)
{
	...
	//  Fetch the scroll area width and height set by setScrollRect()
	uint32_t w = _sw;  // line width to copy
	uint32_t h = _sh - absdy;  // lines to copy
	...
	if(_bpp == 1)
	{
		...
		if(dy <= 0)
		{
			...
			while(w--)
			{
				...
				{
					drawPixel(tx, ty + yp, readPixel(fx, fy + yp));
				}
				tx++;
				fx++;
			}
			...
			for(uint16_t iy = 0; iy < absdy; iy ++)
			{
				for(uint16_t ix = 0; ix < ww; ix ++)
				{
					drawPixel(ix, h + iy, wrap[ix][iy]);
				}
			}	
		}
		else
		{
			...
			for(uint16_t iy = 0; iy < absdy; iy ++)
			{
				for(uint16_t ix = 0; ix < w; ix ++)
				{
					wrap[ix][iy] = readPixel(ix, h + iy);
				}
			}
			//  scroll
			while(w--)
			{
				for(uint16_t yp = absdy - dy; yp <= h; yp++)
				{
					drawPixel(tx, ty - yp, readPixel(fx, fy - yp));
				}
				tx++;
